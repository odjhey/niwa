...
- For the watchdog: macOS and `/bin/zsh`, including the `zsh/system` module and `zsystem flock`, plus `date`, `lsof`, `pgrep`, `ps`, `pwd`, `rm`, `sleep`, `stat`, `tr`, and `wc`. Its `stat -f %m` use is macOS-specific; Linux is not supported by these instructions.
...
|---|---|---|
| `WD_TRANSCRIPT` | Newest matching `*.jsonl` in `~/.claude/projects/<cwd-slug>/`, or empty if none | This watchtower session's transcript for heartbeat idleness. Selects the newest matching transcript when unset, or resolves to empty (disabling heartbeats only) when none exist. Explicit paths are used verbatim. |
...
- Run the watchdog from the intended working directory: default transcript discovery derives `<cwd-slug>` from that directory. When `WD_TRANSCRIPT` is unset and no matching default transcripts exist, the transcript resolves to empty, which disables heartbeat evaluation only; every external lane sensor still runs. Set `WD_TRANSCRIPT` explicitly when testing in directories without Claude sessions.
